    ...
    computed: {
        ...
        classObjectSubItem: function() {
            if (!this.isInfo && this.getRollsLeft === 3 && this.it.locked === true) {
                ...
                !this.isInfo &&
                this.it.selectable === true &&
                this.it.unlockable === true &&
                this.showAnimation
            )
                return 'blink-infinite vl'
            else if (!this.isInfo && this.it.locked === false) return 'vl bold orange'
            else if (!this.isInfo && this.it.selectable === true && this.showAnimation)
                return
    ...
    methods: {
        ...
        startResizeListener() {
            ...
            window.addEventListener('resize', () => {
                if (current.$store.state.isMobile === false && window.innerWidth <= 870) {
                    store.commit('toggleIsMobile', true)
                } else if (
